...
certificats X.509 et deux clients ACME. L'historique [[letsencrypt/acme_tiny/acme_tiny.py]],
disponible sur [github](https://github.com/diafygi/acme-tiny), dont la
documentation est [[letsencrypt/acme_tiny/README]] et le nouveau venu [acme.sh](https://github.com/acmesh-official/acme.sh).
...
règles sudoers de l'utilisateur acme)
...
1. Configuration
   ...
   acme.sh tourne sous l'utilisateur 'acme'. Bien pensez à exécuter les commandes acme.sh en tant que cet utilisateur. Si vous utilisez `sudo -u acme`, le script acme.sh le détectera et ne fera rien.

   Il faut enregistrer un compte pour utiliser acme avec ZeroSSL, mais on préfère Let's Encrypt
   donc pas besoin de `acme.sh --register-account`, on passe directement à la création du certificat ci-dessous.

1. Création de clé & demande de certificat
   ...
   certain de la conf parce que Lets Encrypt est radin en retry...

1. Faire utiliser les nouveaux certifs/clés au service que l'on veut configurer
   ...
   Le certificat est lisible par tout le monde: faire un lien du service vers
...
1. Tester que le service marche bien

   Verifier que `acme.sh --cron` retrouve bien ses petits ;
   s'il y a un service à redémarrer rajouter `--force` pour tester le redémarrage du service (indiqué avec l'option `--renew-hook`). Vous pouvez voir les certificats créés et les prochains renouvellement avec `acme.sh --list`.
...
1. Création des clés et demande de certification
    ...
    > (belette : si erreur de type `Can't load /root.rnd into RNG` cela provient probablement de la version OpenSSL 1.1.1 11 Sep 2018, contournement possible -> `touch /root/.rnd`)

1. Configuration du serveur web pour acme
...
1. Configuration du serveur web pour ce certificat
...
1. On relit la configuration d'apache :
